...
@pytest.mark.asyncio
async def test_analyze_endpoint_mock(client, mock_groq_agent):
    mock_groq_agent.analyze_schema.return_value = "Mock Health Report"
    ...
    mock_groq_agent.analyze_schema.assert_called_once()
...
@pytest.mark.asyncio
async def test_rename_endpoint_mock(client, mock_groq_agent):
    mock_groq_agent.semantic_rename.return_value = {"old_col": "new_col"}
    ...
    mock_groq_agent.semantic_rename.assert_called_once()

@pytest.mark.asyncio
async def test_normalize_endpoint_mock(client, mock_groq_agent):
    mock_groq_agent.analyze_normalization.return_value = {"god_tables": [{"table": "users"}], "recommendations": ["split users"]}
    ...
    mock_groq_agent.analyze_normalization.assert_called_once()

@pytest.mark.asyncio
async def test_modernize_endpoint_mock_no_validation(client, mock_groq_agent):
    mock_groq_agent.semantic_rename.return_value = {"old_name": "new_name"}
    mock_groq_agent.analyze_normalization.return_value = {"god_tables": [], "recommendations": []}
    mock_groq_agent.generate_modernized_ddl.return_value = "CREATE TABLE new_table (id INT);"
    ...
    mock_groq_agent.semantic_rename.assert_called_once()
    mock_groq_agent.analyze_normalization.assert_called_once()
    mock_groq_agent.generate_modernized_ddl.assert_called_once()

@pytest.mark.asyncio
async def test_modernize_endpoint_mock_with_validation_success(client, mock_groq_agent):
    mock_groq_agent.semantic_rename.return_value = {"old_name": "new_name"}
    mock_groq_agent.analyze_normalization.return_value = {"god_tables": [], "recommendations": []}
    mock_groq_agent.generate_modernized_ddl.return_value = "CREATE TABLE new_table (id INT);"
...
@pytest.mark.asyncio
async def test_modernize_endpoint_mock_with_validation_failure_and_healing(client, mock_groq_agent):
    mock_groq_agent.semantic_rename.return_value = {"old_name": "new_name"}
    mock_groq_agent.analyze_normalization.return_value = {"god_tables": [], "recommendations": []}
    mock_groq_agent.generate_modernized_ddl.return_value = "CREATE TABLE new_table (id INT);"
    mock_groq_agent.fix_sql_errors.return_value = "CREATE TABLE new_table (id INT, fixed_col TEXT);" # LLM fixes it
    ...
    with patch("src.main.SQLParser.validate_sql_syntax", new_callable=AsyncMock) as mock_validate:
        ...
        mock_groq_agent.fix_sql_errors.assert_called_once()

@pytest.mark.asyncio
async def test_modernize_endpoint_max_retries_exhausted(client, mock_groq_agent):
    """Verifies that the API fails gracefully when SQL cannot be healed."""
    mock_groq_agent.generate_modernized_ddl.return_value = "CREATE TABLE error (id INT);"
...
@pytest.mark.asyncio
async def test_prompt_injection_protection(client, mock_groq_agent):
    """
    ...
    mock_groq_agent.analyze_schema.return_value = "I cannot fulfill this request due to security boundaries."
    response = await client.post("/analyze", files=files)
...
@pytest.mark.asyncio
async def test_global_exception_handler_masking(client, mock_groq_agent):
    """
    ...
    mock_groq_agent.analyze_schema.side_effect = RuntimeError("Database driver crashed!")
...
@pytest.mark.asyncio
async def test_generate_migration_endpoint_mock(client, mock_groq_agent):
    mock_groq_agent.generate_migration_script.return_value = "INSERT INTO new_table SELECT * FROM old_table;"
    ...
    mock_groq_agent.generate_migration_script.assert_called_once()

@pytest.mark.asyncio
async def test_path_traversal_protection(client, mock_groq_agent):
    """
    Verifies that malicious path traversal attempts in filenames are sanitized.
    """
    mock_groq_agent.analyze_schema.return_value = "Mock Report"
...
@pytest.mark.asyncio
async def test_rate_limiting_active(client, mock_groq_agent):
    """
